...
use codexmate_core::session::{DeleteResult, ExportResult, SessionRef};
use codexmate_core::routes::{BridgeContext, BridgeDataService, BridgeRuntimeService};
...
fn acquire_single_instance_guard() -> anyhow::Result<Option<std::net::TcpListener>> {
    match codexmate_core::ports::acquire_loopback_port_guard(
        codexmate_core::ports::LAUNCHER_GUARD_PORT,
    ) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AddrInUse => {
            let _ = codexmate_core::diagnostic_log::append_diagnostic_log(
                ...
                json!({
                    "guard_port": codexmate_core::ports::LAUNCHER_GUARD_PORT
                }),
            );
            Ok(None)
        }
...
async fn notify_manager_when_update_available() -> anyhow::Result<bool> {
    let update = codexmate_core::update::check_for_update(codexmate_core::version::VERSION).await?;
...
fn open_manager_with_update_prompt() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        command.creation_flags(codexmate_core::windows_create_no_window());
    }
    command
        .spawn()
        .map(|_| ())
...
impl LaunchHooks for LauncherHooks {
    fn resolve_app_dir(
        ...
        settings: &codexmate_core::settings::BackendSettings,
    ...
    async fn load_settings(&self) -> anyhow::Result<codexmate_core::settings::BackendSettings> {
        self.core.load_settings().await
    }

    async fn run_provider_sync(&self) -> anyhow::Result<()> {
        let _ = tokio::task::spawn_blocking(|| codexmate_data::run_provider_sync(None))
            .await
    ...
    async fn launch_codex(
        ...
        debug_port: u16,
        extra_args: &[String],
    ) -> anyhow::Result<codexmate_core::launcher::CodexLaunch> {
        self.core
    ...
    async fn wait_for_codex_exit(
        &self,
        launch: &codexmate_core::launcher::CodexLaunch,
    ) -> anyhow::Result<()> {
    ...
    async fn terminate_codex(&self, launch: &codexmate_core::launcher::CodexLaunch) {
...
impl Default for LauncherDataService {
    fn default() -> Self {
        Self {
            ...
            backup_dir: codexmate_core::paths::default_app_state_dir().join("backups"),
        }
    }
}
...
impl BridgeDataService for LauncherDataService {
    ...
    async fn export_markdown(&self, session: SessionRef) -> anyhow::Result<ExportResult> {
        let export_service = codexmate_data::MarkdownExportService::new(Some(self.db_path.clone()));
...
impl LauncherDataService {
    fn storage_adapter(&self) -> codexmate_data::SQLiteStorageAdapter {
        codexmate_data::SQLiteStorageAdapter::new(
            self.db_path.clone(),
            codexmate_data::BackupStore::new(self.backup_dir.clone()),
        )
    }
}
...
impl BridgeRuntimeService for LauncherRuntimeService {
    async fn open_devtools(&self) -> anyhow::Result<Value> {
        ...
        let targets = codexmate_core::cdp::list_targets(debug_port).await?;
        let target = codexmate_core::cdp::pick_page_target(&targets)?;
        let url = codexmate_core::routes::devtools_url(debug_port, &target.id);
        open_url(&url)?;
        Ok(json!({
            "status": "ok",
            "target_id": target.id,
    ...
    async fn open_manager(&self) -> anyhow::Result<Value> {
        ...
        #[cfg(windows)]
        {
            std::process::Command::new(&manager_path)
                .creation_flags(codexmate_core::windows_create_no_window())
                .spawn()
                .map_err(|error| anyhow::anyhow!("启动管理工具失败：{error}"))?;
        }
        #[cfg(not(windows))]
...
async fn try_inject_with_context(
    ...
    let script = codexmate_core::assets::injection_script(helper_port);
    codexmate_core::bridge::install_bridge(
        websocket_url,
        codexmate_core::bridge::BRIDGE_BINDING_NAME,
        Arc::new(move |path, payload| {
            let ctx = ctx.clone();
            Box::pin(async move {
                Ok(codexmate_core::routes::handle_bridge_request(ctx, &path, payload).await)
            })
        }),
        &[script],
    )
...
fn manager_exe_path() -> PathBuf {
    ...
    dir.join(format!(
        "{}{}",
        codexmate_core::install::MANAGER_BINARY,
        suffix
    ))
}
    ...
    fn manager_update_prompt_uses_sidecar_manager_binary_name() {
        ...
        assert!(
            path.file_name()
                ...
                .is_some_and(|name| name.contains(codexmate_core::install::MANAGER_BINARY))
        );
    }
}
