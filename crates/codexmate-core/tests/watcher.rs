...
fn watcher_install_plan_registers_rust_launcher_at_logon() {
    let plan = build_watcher_install_plan("C:/Tools/codexmate.exe".into(), 9333);

    assert_eq!(plan.run_value_name, "CodexMateWatcher");
    assert_eq!(
        plan.run_value,
        "\"C:/Tools/codexmate.exe\" --debug-port 9333"
    );
    assert_eq!(plan.shortcut_name, "CodexMateWatcher.lnk");
    assert_eq!(plan.shortcut_target, "C:/Tools/codexmate.exe");
    assert_eq!(plan.shortcut_arguments, "--debug-port 9333");
}

#[test]
fn spawn_launcher_command_points_to_silent_binary_only() {
    let command = build_spawn_launcher_command("C:/Tools/codexmate.exe", 9444);

    assert_eq!(command[0], "C:/Tools/codexmate.exe");
