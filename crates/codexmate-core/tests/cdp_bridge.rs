use codexmate_core::assets;
use codexmate_core::bridge::{self, BRIDGE_BINDING_NAME};
use codexmate_core::cdp::{CdpTarget, pick_page_target};
...
#[test]
fn injection_script_prefixes_helper_url_and_version_globals() {
    let script = assets::injection_script(57321);

    assert!(script.contains("window.__CODEX_SESSION_DELETE_HELPER__"));
    assert!(script.contains("http://127.0.0.1:57321"));
    assert!(script.contains("window.__CODEXMATE_VERSION__"));
    assert!(script.contains(codexmate_core::version::VERSION));
}
...
fn injection_script_marks_diagnostic_build_and_reports_script_loaded() {
    ...
    assert!(script.contains("window.__CODEXMATE_BUILD__"));
    assert!(script.contains(codexmate_core::assets::DIAGNOSTIC_BUILD_ID));
    assert!(script.contains("script_loaded"));
}

#[test]
fn injection_script_routes_scoped_model_requests_through_helper() {
    let script = assets::injection_script(57321);

    assert!(script.contains("routedHelperResponsesUrl"));
    assert!(script.contains("\"/v1/responses\""));
    assert!(script.contains("findScopedModelPayload"));
    assert!(script.contains("installAppServerModelRequestPatch"));
    assert!(script.contains("installXhrModelRequestPatch"));
}

#[test]
fn injection_script_exposes_backend_status_indicator() {
    let script = assets::injection_script(57321);

    assert!(script.contains("checkBackendStatus"));
    assert!(script.contains("openManager"));
    assert!(script.contains("/backend/status"));
    assert!(script.contains("/manager/open"));
    assert!(script.contains(".codex-backend-status"));
    assert!(script.contains("代理模式"));
    assert!(script.contains("直连模式"));
    assert!(script.contains("setInterval(poll, 15000)"));
}

#[test]
fn injection_script_bootstraps_scan_and_diagnostics() {
    let script = assets::injection_script(57321);
    let scan_call = script
        .rfind("scan();")
        .expect("script should perform an initial scan");
    let footer = &script[scan_call..];
    let diagnostics_call = footer
        .find("sendDiag(\"script_loaded\"")
        .expect("script should report that it loaded");
    let footer_end = footer
        .find("})();")
        .expect("script should terminate cleanly after diagnostics");

    assert!(diagnostics_call < footer_end);
    assert!(script.contains("scanLightweight"));
    assert!(script.contains("scanDeferred"));
    assert!(script.contains("sendDiag(\"script_loaded\""));
}

#[test]
fn injection_script_patches_model_list_messages() {
    let script = assets::injection_script(57321);

    assert!(script.contains("codex-message-from-view"));
    assert!(script.contains("mcp-request"));
    assert!(script.contains("model/list"));
    assert!(script.contains("includeHidden: true"));
    assert!(script.contains("patchMsgData"));
}
...
fn injection_script_unlocks_custom_model_catalog() {
    ...
    assert!(script.contains("modelCatalog"));
    assert!(script.contains("patchModelArray"));
    assert!(script.contains("patchStatsig"));
...
#[test]
fn injection_script_bootstraps_scan_after_diagnostics_helpers_are_ready() {
    let script = assets::injection_script(57321);
    let post_json = script
        .find("function postJson")
        .expect("script should define bridge post helper");
    let scan_call = script
        .find("scan();")
        .expect("script should perform an initial scan");

    assert!(post_json < scan_call);
}
