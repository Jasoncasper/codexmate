...
pub trait LaunchHooks: Send + Sync {
    ...
    async fn sync_model_provider_for_current_model(
        &self,
        helper_port: u16,
    ) -> anyhow::Result<()> {
        crate::relay_config::sync_live_model_provider_for_model(
    ...
    H: IntoLaunchHooks,
{
    ...
    let result: anyhow::Result<LaunchHandle> = async {
        ...
        hooks
            .sync_model_provider_for_current_model(helper_port)
            .await?;
...
impl LaunchHooks for DefaultLaunchHooks {
    ...
    async fn run_provider_sync(&self) -> anyhow::Result<()> {
        anyhow::bail!("provider sync requires launcher hooks with codexmate-data integration")
    }
