...
use crate::router::config::{
    SmartProvider, SmartRouterConfig, normalize_router_config, scoped_model_id,
};
use serde_json::{Value, json};

const BASE_URL_ENV_KEYS: &[&str] = &[
    "CODEXMATE_OPENAI_BASE_URL",
    "CODEXMATE_BASE_URL",
    "OPENAI_BASE_URL",
    "OPENAI_API_BASE_URL",
    "OPENAI_API_BASE",
...
pub async fn read_codex_model_catalog() -> Value {
    ...
    let client = match crate::http_client::proxied_client("CodexMate/1.0") {
        Ok(client) => client,
        Err(error) => {
            return json!({
                "status": "failed",
...
pub async fn read_codex_model_catalog_from_home(
    ...
    read_codex_model_catalog_from_home_with_app_state(
        home,
...
pub async fn read_codex_model_catalog_from_home_with_app_state(
    home: &Path,
    app_state_dir: &Path,
    env: &HashMap<String, String>,
    client: reqwest::Client,
    ...
    let router_config_path = app_state_dir.join("routing.toml");
    ...
    if let Ok(contents) = std::fs::read_to_string(&router_config_path) {
        if let Ok(router_config) = toml::from_str::<SmartRouterConfig>(&contents) {
            let router_config = normalize_router_config(router_config);
            for provider in &router_config.providers {
                if provider.enabled && !provider.id.trim().is_empty() {
                    router_models.push(scoped_model_id(provider));
                }
            }
        }
    }
    ...
    if !router_models.is_empty() {
        ...
        for source in sources.iter() {
            let (source_models, mut source_status) =
                fetch_models_from_source(&client, source).await;
            source_status["responses_api"] = responses_api_status("unknown", "", "");
            models.extend(source_models);
            source_statuses.push(source_status);
        }
        let (catalog_models, catalog_status) =
            models_from_config_model_catalog_json(home, &effective);
...
pub async fn fetch_provider_model_ids(
    ...
    let endpoint = models_endpoint(&source.base_url);
    let client = crate::http_client::proxied_client("CodexMate/1.0")?;
