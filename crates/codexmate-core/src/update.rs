...
fn normalize_github_repository(value: &str) -> Option<&str> {
    ...
    trimmed
        ...
        .or_else(|| {
            ...
            if slash_count == 1 { Some(trimmed) } else { None }
        })
}
...
pub async fn fetch_latest_release(latest_json_url: &str) -> anyhow::Result<Release> {
    let client =
        crate::http_client::proxied_client(&format!("CodexMate/{}", crate::version::VERSION))?;
    let payload = client
        .get(latest_json_url)
...
fn is_macos_installer_asset(name: &str) -> bool {
    name.contains("codexmate") && name.ends_with(".dmg")
}
