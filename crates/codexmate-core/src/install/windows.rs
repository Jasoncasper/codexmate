...
#[allow(dead_code)]
const UNINSTALL_SUBKEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\CodexMate";
#[allow(dead_code)]
...
pub fn build_windows_entrypoint_plan(options: &InstallOptions) -> WindowsEntrypointPlan {
    ...
    WindowsEntrypointPlan {
        ...
        uninstall_key: "CodexMate".to_string(),
        legacy_uninstall_key: "CodexMate".to_string(),
...
fn write_uninstall_registration(plan: &WindowsEntrypointPlan) -> anyhow::Result<()> {
    ...
    let install_location = Path::new(&plan.manager_path)
        ...
        .to_string();
    for (name, value) in [
        ("DisplayName", "CodexMate".to_string()),
        ("DisplayVersion", crate::version::VERSION.to_string()),
        ("Publisher", "CodexMate contributors".to_string()),
...
fn default_icon_path() -> PathBuf {
    std::env::current_exe()
        ...
        .map(|path| path.join("codexmate.ico"))
        .unwrap_or_else(|| PathBuf::from("codexmate.ico"))
}
