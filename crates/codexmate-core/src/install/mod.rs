...
pub const SILENT_NAME: &str = "CodexMate";
pub const MANAGER_NAME: &str = "CodexMate";
pub const SILENT_BINARY: &str = "codexmate-manager";
pub const MANAGER_BINARY: &str = "codexmate-manager";
...
pub fn shortcut_names() -> (&'static str, &'static str) {
    ("CodexMate.lnk", "CodexMate.lnk")
}

pub fn app_bundle_names() -> (&'static str, &'static str) {
    ("CodexMate.app", "CodexMate.app")
}
...
fn platform_install(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持安装 CodexMate 入口")
    }
}
...
fn platform_uninstall(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持卸载 CodexMate 入口")
    }
}
