use codexmate_data::{ProviderSyncStatus, run_provider_sync};
...
#[test]
fn provider_sync_adds_missing_workspace_root_from_thread_cwd() {
    let tmp = tempdir().unwrap();
    let home = tmp.path().join(".codex");
    fs::create_dir(&home).unwrap();
    fs::write(home.join("config.toml"), "model_provider = \"apigather\"\n").unwrap();
    ...
    fs::write(
        home.join(".codex-global-state.json"),
        json!({
            "electron-saved-workspace-roots": ["/Users/admins/codexplus"],
            "project-order": ["/Users/admins/codexplus"],
            "active-workspace-roots": "/Users/admins/codexplus",
            "electron-workspace-root-labels": {"/Users/admins/codexplus": "codexplus"}
        })
        .to_string(),
    )
    .unwrap();
...
fn provider_sync_skips_when_home_missing_or_lock_exists_and_prunes_backups() {
    ...
    for index in 0..6 {
        ...
        fs::write(
            ...
            json!({"managedBy": "CodexMate provider sync"}).to_string(),
        )
        .unwrap();
    }
    write_rollout(
