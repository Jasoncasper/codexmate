...
fn create_backup(
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(
            &json!({"managedBy": "CodexMate provider sync", "targetProvider": target_provider}),
        )?,
    )?;
    Ok(backup_dir)
}
...
fn count_global_state_updates(
    path: &Path,
    cwd_by_thread_id: &HashMap<String, String>,
) -> anyhow::Result<usize> {
    let state = load_global_state(path)?;
    let next = normalized_global_state(&state, cwd_by_thread_id);
    Ok(next
        .iter()
...
fn prune_backups(home: &Path) -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(&root)? {
        ...
        if value.get("managedBy").and_then(Value::as_str) == Some("CodexMate provider sync") {
            managed.push(path);
        }
    }
